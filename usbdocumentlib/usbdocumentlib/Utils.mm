...
void usbdocument::Utils::error_name(IOReturn err, char *out_buf) {
    switch (err) {
        ...
        default: sprintf(out_buf,"Unknown Error:%d Sub:%d System:%d",err_get_code(err),
                         err_get_sub(err),err_get_system(err)); break;
    }
}
void usbdocument::Utils::print_error(IOReturn err) {
    char buf[512];
    memset(buf,0,512*sizeof(char));
    Utils::error_name(err,buf);
    printf("%s\n",buf);
}
IOReturn usbdocument::Utils::watch_error(IOReturn err) {
    if(err!=kIOReturnSuccess) {
        Utils::print_error(err);
    }
    return err;
}
IOReturn usbdocument::Utils::assert_error(IOReturn err) {
    if(err!=kIOReturnSuccess) {
        Utils::print_error(err);
        throw "Assertion failed (IOReturn != kIOReturnSuccess)";
    }
    return err;
}
void usbdocument::Utils::test_assert(bool result,std::string message) {
    if(!result) throw message;
}
#define NANOS_IN_SECOND (1000*1000*1000)
void usbdocument::Utils::milli_sleep(unsigned long milliseconds) {
    struct timespec tim;
    unsigned long seconds=milliseconds/1000;
    unsigned long nanoseconds=(milliseconds*1000*1000)-seconds*NANOS_IN_SECOND;
    tim.tv_sec=seconds;
